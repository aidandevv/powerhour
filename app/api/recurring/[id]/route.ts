...
export async function PATCH(
  ...
  try {
    ...
    if (parsed.data.name !== undefined) updateData.name = encryptField(parsed.data.name) ?? parsed.data.name;
    ...
    return NextResponse.json({
      ...updated,
      name: decryptField(updated.name) ?? updated.name,
      merchantName: decryptField(updated.merchantName),
    });
  } catch (error: unknown) {
    return apiError(error, "Failed to update recurring item");
  }
}
